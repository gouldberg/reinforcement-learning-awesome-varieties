...
# common
# unpack_batch
#  - It takes the batch of transitions and converts it into the se of NumPy arrays suitable for training.
#  - Every transition from ExperienceSourceFirstLast has a type pf ExperienceFirstLast, which is a namedtuple with the following fields:
#     - state: observation from the environment
#     - action: integer action taken by the agent
#     - reward: if we have created ExperienceSourceFirstLast with the attribute steps_count=1,
#               it's just the immediate reward. For larger step counts, it contains the discounted sum of rewareds
#               for this number of steps.
#     - last_state: if the transition corresponds to the final step in the environment, then this field is None;
# #                 otherwise, it containes the last observation in the experience chain. 
# ------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------------
# common
# Calculation of the DQN loss function
# ------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------------
# common
# implements epsilon decay during training
# epsilon: probability of taking the random action by the agent.
# ------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------------
# common
# infinitely generates training batches sampled from the buffer.
# ------------------------------------------------------------------------------------------------------
...
model_path = os.path.join(base_path, '04_output/pong/deep_q_learning_ptan/model/PongNoFrameskip-v4-01_baseline.pth')
...
# # record_path = os.path.join(base_path, '04_output/pong/deep_q_learning_ptan/video/PongNoFrameskip-v4-01_baseline')
# record_path = os.path.join(base_path, '04_output/pong/deep_q_learning_ptan/video/PongNoFrameskip-v4-01_baseline_video.mp4')
...
#     vid.capture_frame()
#     # ----------
#     state_v = torch.tensor(np.array([state_reset], copy=False))
